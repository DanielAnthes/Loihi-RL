...
dt = 0.001
duration = 600
env = TestEnvActor(dt=dt, trial_length=60)
discount = 0.98622

with nengo.Network() as net:
    envnode = nengo.Node(lambda t, v: env.step(v), size_in=1, size_out=3)
    in_ens = nengo.Ensemble(n_neurons=1000, radius=1, dimensions=1)  # encodes position
    #critic = CriticNet(in_ens, n_neuron_out=1000, lr=1e-5)
    critic = nengo.Node(lambda x, p: discount ** (abs(p) / env.stepsize), size_in=1, size_out=1)
    ...
    nengo.Connection(critic, error.net.errornode[1], synapse=0) # value prediction
    ...
    criticprobe = nengo.Probe(critic, sample_every=0.5)
